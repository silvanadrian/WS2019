...
class Vocab(object):
    ...
    def __init__(self, unk=DEFAULT_UNK, emb=None, init_from_embeddings=False):
        ...
        if init_from_embeddings and emb is not None:
            self.sym2id = {v: k for k, v in enumerate(emb.index2word)} #emb.vocabulary.word2idx.items()}
            self.id2sym = {v: k for k, v in self.sym2id.items()} #dict(emb.vocabulary.word2idx)
